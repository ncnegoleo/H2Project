...
public class RecursoSala {
	...
	/**
	 * Este método serve controlador para a adição de novas salas no
	 * sistema.
	 * 
	 * @param idSala
	...
	public void addSala(String idSala, String bloco) throws H2Exception {
		...
		// valida se a entrada do id da sala esta de acordo com as regras.
		if (H2Validation.validaCampos(idSala, "Atributo inválido"))
			sala.setId(idSala);
		
		// Valida se a entrada do bloco da sala esta de acordo com as regras.
		if (H2Validation.validaCampos(bloco, "Atributo inválido"))
			sala.setBloco(bloco);
		
		// verifica se não existe professores com a mesma id.
		if (AbstractFactoryDao.createSalaDaoIF().getSalaById(
				sala.getId()) == null) {
	...
	/**
	 * Este método serve como controlador para a alteração de uma sala na
	 * base de dados
	 * 
	 * @param idSala
	 *            Identificador da Sala.
	...
	public void alteraSala(String idSala, String novoBloco) throws H2Exception {
		...
		// valida se a entrada do novo bloco esta de acordo com as regras.
		if (H2Validation.validaCampos(novoBloco, "Atributo inválido"));
		...
		if (H2Validation.validaObjetosNaoNulos(sala, "Sala não cadastrada"));
	...
	/**
	 * Este método como controlador para a remoção de salas na base de
	 * dados
	 * 
	 * @param idSala
	 *            Identificador da Sala.
	...
	public String getSala(String idSala) throws H2Exception {
		...
		// valida se a entrada do id esta de acordo com as regras.
		if (H2Validation.validaCampos(idSala, H2ErrorMessages.ATRIBUTOINVALIDO.getValor()));
		...
		if (H2Validation.validaObjetosNaoNulos(sala, H2ErrorMessages.SALANAOCADASTRADA.getValor()));
